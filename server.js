...
const specialTrim = function specialTrim(unTrimmedString) {
  return unTrimmedString.replace(/\t/gim, " ").trim().replace(/\s+/gim, " ");
};
...
const nameToIdentifier = function nameToIdentifier(name) {
  return name.toLowerCase().replace(/['`]/gim, "").replace(/\s/gim, "-");
};
      ...
      (error, response, body) => {
        ...
        $(".table-list__row").each((index, element) => {
          ...
          if ($currentElement.prev().hasClass("title")) {
            ...
            currentRestaurantLink = `http://www.lindholmen.se${$title
              .find("a")
              .attr("href")}`;

            if (currentRestaurantName.indexOf("(") > -1) {
              currentRestaurantName = currentRestaurantName.substr(
                0,
                currentRestaurantName.indexOf("(")
              );
            }

            // Special case for '
            currentRestaurantName = currentRestaurantName.replace(
              /&#039;/gim,
              "'"
            );
          ...
          currentDishes[nameToIdentifier(currentRestaurantName)].dishes.push(
            parseRow($(element))
          );
        });
...
const getSlackMessageForRestaurant = function getSlackMessageForRestaurant(
  restaurantName
) {
  const responseData = {
    attachments: [],
...
app.all("/slack", loadRestaurants, (webRequest, response) => {
  ...
  if (!restaurant && webRequest.body && webRequest.body.text) {
    const parsedText = webRequest.body.text.replace(
      webRequest.body.trigger_word,
      ""
    );
...
app.listen(process.env.PORT || DEFAULT_PORT, () => {
  console.log(
    "Service up and running on port",
